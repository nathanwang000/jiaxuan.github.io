...
var Game = new function(){
	...
	this.music = { title:document.getElementById("title_music"),
		       victory:document.getElementById(
			   "victory_music"),
    		       defeat:document.getElementById(
			   "defeat_music"),
    		       battle:document.getElementById(
			   "battle_music") }
	this.sound = { shoot:document.getElementById("shoot_sound"),
		       hit:document.getElementById("hit_sound"),
		     }
	...
	for (var piece in this.music)
	    this.music[piece].pause();
    }
...
// get hit, remove itself from the game board
Sprite.prototype.hit = function(damage) {
    this.board.remove(this);
}

LivingObject = function() { this.alpha = 1; }
LivingObject.prototype = new Sprite();
LivingObject.prototype.setup = function(sprite, props) {
    this.sprite = sprite;
    this.w = SpriteSheet.map[sprite].w;
    this.h = SpriteSheet.map[sprite].h;
    this.totalHealth = SpriteSheet.map[sprite].totalHealth;
    this.health = this.totalHealth;
    this.merge(props);
    this.frame = this.frame || 0;
};
LivingObject.prototype.hit = function(damage) {
    Game.sound.hit.play();
    this.health -= damage;
    if (this.health <= 0) {
	this.board.remove(this);
	if (this.type == OBJECT_ENEMY)
	    Game.points += this.points || 100;
	this.board.add(new Explosion(this.x + this.w/2,
				     this.y + this.h/2));
    }
}
LivingObject.prototype.healthBar = { width: 40,
				     height: 5,
				     padding: 5 }
LivingObject.prototype.draw = function(ctx) {
    SpriteSheet.draw(ctx, this.sprite, this.x, this.y, this.frame);
	...
	+ this.alpha + ")";
    ctx.lineWidth = 2;
    var h_b_x = this.x + this.w/2 - this.healthBar.width/2;
    var h_b_y = this.y - this.healthBar.padding
	- this.healthBar.height;
    var factor = this.health / this.totalHealth;
    ctx.strokeRect(h_b_x, h_b_y, this.healthBar.width,
		   this.healthBar.height);
    ctx.fillRect(h_b_x, h_b_y, factor*this.healthBar.width,
		 this.healthBar.height);
}
LivingObject.prototype.inbound_left = function() {
    if (this.x < 0) { 
	this.x = 0;
	this.vx = -this.vx;
    }
}
LivingObject.prototype.inbound_right = function() {
